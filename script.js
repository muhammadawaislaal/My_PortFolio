...
const initChatbot = () => {
  ...
  const getBotResponse = (input) => {
    ...
    if (q.includes('hi') || q.includes('hello') || q.includes('hey')) {
      return "Hello! I'm the assistant for Muhammad Awais Laal. I can help you with details about his projects, skills, and how to hire him. What would you like to know?";
    }

    // Projects
    if (q.includes('project') || q.includes('work')) {
      return "Awais has built several high-impact AI projects, including a <strong>Course Management System</strong>, an <strong>AI SQL Agent</strong>, and a <strong>Trading Signal Predictor</strong>. You can view them all in the Portfolio section!";
    }

    // About/Profile
    if (q.includes('about') || q.includes('who is awais') || q.includes('profile')) {
      return "Muhammad Awais Laal is a dedicated <strong>Gen AI Developer</strong> from Pakistan. He specializes in building intelligent applications using Flask, Python, and LangChain. He's passionate about solving complex problems with AI.";
    }

    // Skills
    if (q.includes('skill') || q.includes('expert') || q.includes('technolog')) {
      return "Awais is highly skilled in <strong>Python, Flask, LangChain, Transformers, FineTuning, and SQL</strong>. He also has extensive experience with WordPress and AI Business Automation.";
    }

    // Contact/Hire
    if (q.includes('contact') || q.includes('hire') || q.includes('job') || q.includes('fiverr') || q.includes('linkedin')) {
      return "You can hire Awais directly through his <a href='https://www.fiverr.com/pooorman?public_mode=true' target='_blank'>Fiverr Profile</a> or connect on <a href='https://linkedin.com/in/muhammad-awais-2a3450324/' target='_blank'>LinkedIn</a>. For direct inquiries, email him at <a href='mailto:muhammadawaislaal@gmail.com'>muhammadawaislaal@gmail.com</a>.";
    }

    // Price
    if (q.includes('price') || q.includes('cost') || q.includes('charge')) {
      return "For pricing and project estimates, it's best to discuss your specific requirements. You can reach out via Fiverr or email to get a custom quote!";
    }

    // Human mode switch request
    if (q.includes('human') || q.includes('talk to you') || q.includes('real person')) {
      return "I can certainly help you connect with Awais directly! Just click the 'Switch to Human' button or ask me to switch.";
    }

    return "That's an interesting question! While I'm focused on providing info about my boss's professional profile, I might not have the answer to that. Please contact him at <strong>muhammadawaislaal@gmail.com</strong> or book a meeting through Fiverr for a detailed discussion.";
  };
  ...
  const handleAISwitch = () => {
    ...
    setTimeout(() => {
      thinking.remove();
      addMessage("I'm back! I'm the assistant for Awais. How can I help you explore his portfolio further?", 'bot');
    }, 1000);
    ...
    btns.forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        if (text === 'Switch to Human') {
          ...
          setTimeout(() => {
            thinking.remove();
            addMessage("I'd love to hear more! Please send the project details to <a href='mailto:muhammadawaislaal@gmail.com'>muhammadawaislaal@gmail.com</a> or message me on Fiverr.", 'bot');
          }, 1500);
          ...
          setTimeout(() => {
            thinking.remove();
            const response = isHumanMode ?
              "I've noted that! Since I'm in human mode, it's best if we continue via email or Fiverr for a faster real-time response. Would you like to do that?" :
              getBotResponse(text);
