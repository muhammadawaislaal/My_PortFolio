...
// Groq AI API Integration - Obfuscated Configuration
const GROQ_API_URL = 'https://api.groq.com/openai/v1/chat/completions';

// Load API Key from obfuscated configuration
let GROQ_API_KEY = null;

// Initialize API key on page load
const loadAPIKey = () => {
  if (window.ApiConfig) {
    GROQ_API_KEY = window.ApiConfig.init();
    if (GROQ_API_KEY) {
      console.log('API configuration loaded successfully');
    } else {
      console.warn('API configuration unavailable. Chatbot will use fallback mode.');
    }
  } else {
    console.error('ApiConfig not loaded. Please ensure api-config.js is included before script.js');
  }
  return GROQ_API_KEY;
};
